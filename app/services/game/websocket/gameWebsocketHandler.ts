...
const gameWebsocketHandler = async (
  ...
  const player = new Player(request.userId, connection, request.userName);
  ...
  connection.on("message", async (message) => {
    try {
      ...
      if (
        ...
        player.currentState === "WaitingForMessage"
      )
        matchMaking.createMatch(
          player,
          convertMessageToMatchOptions(jsonMessage),
          db,
        );
      else if (
        messageType === "joinRandomMatch" &&
        player.currentState === "WaitingForMessage"
      )
        matchMaking.asignUserToRandomMatch(
          player,
          convertMessageToMatchOptions(jsonMessage),
          db,
        );
      else if (
        messageType === "joinMatchWithId" &&
        player.currentState === "WaitingForMessage"
      )
      ...
      if (error instanceof Error) connection.send(error.message);
    }
  });
};
